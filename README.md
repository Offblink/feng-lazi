...
前台应用使用时长统计工具，**两个版本**：经典自绘版（`pyqt/`）与 Fluent Design 版（`fluent/`，均基于 PyQt6）。常驻系统托盘后台运行，只统计**前台窗口**对应的应用；桌面、任务栏、系统托盘、锁屏等状态不计时。每次连续使用都记录**精确起止时刻**（几时几分到几时几分），一眼看出什么时候在用什么。名字取自《红楼梦》王熙凤的绰号，凤辣子盯得紧，你的每一秒它都记着。
...
两版共享同一套跟踪/存储逻辑（`store`/`session`/`foreground` 同源），界面内容一致三页：**使用时段**甘特图（每款应用一行**固定高度**时间轴，按精确起止时刻渲染，悬停看 起止时刻 + 时长；应用多到超出窗口高度时出现滚动条）、**各应用**使用总时长比例条列表（图标取自 exe）、**近 7 天**每日总时长 + 当日 top 3 应用。
